...
Shader::Shader(Kind kind, std::shared_ptr<Device> device, std::string entry_point):
    kind_(kind),
    device_(device),
...
bool Shader::Create(const char* spv_bytes, size_t byte_size) {
    ...
    return (vkCreateShaderModule(device_->get(), &create_info, nullptr, &shader_module_) == VK_SUCCESS);
}
