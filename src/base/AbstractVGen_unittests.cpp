...
TEST(AbstractVGenTest, DuplicateParameterNames) {
    AbstractVGen dupIn("dupIn", scin::base::AbstractVGen::Rates::kPixel, false, false, { "in1", "in1" }, { "out" },
                       { { 1, 1 } }, { { 1 } }, "@out = @in1 + @in1;");
    EXPECT_FALSE(dupIn.prepareTemplate());
    EXPECT_FALSE(dupIn.valid());

    AbstractVGen dupOut("dupOut", scin::base::AbstractVGen::Rates::kPixel, false, false, {}, { "out", "out" }, { {} },
                        { { 1, 1 } }, "@out = 2.0f;");
    EXPECT_FALSE(dupOut.prepareTemplate());
    EXPECT_FALSE(dupOut.valid());

    AbstractVGen inCrossOut("inCrossOut", scin::base::AbstractVGen::Rates::kPixel, false, false, { "cross" },
                            { "cross" }, { { 1 } }, { { 1 } }, "@cross = @cross;");
    EXPECT_FALSE(inCrossOut.prepareTemplate());
    EXPECT_FALSE(inCrossOut.valid());
}

TEST(AbstractVGenTest, ReservedParameterNames) {
    AbstractVGen timeInput("timeInput", scin::base::AbstractVGen::Rates::kPixel, false, false, { "time" }, { "out" },
                           { { 1 } }, { { 1 } }, "@out = @time;");
...
TEST(AbstractVGenTest, UnknownParameterNames) {
    AbstractVGen noInputParams("noParams", scin::base::AbstractVGen::Rates::kPixel, false, false, {}, { "out" }, { {} },
                               { { 1 } }, "@out = sin(@freq * 2 * pi * @time);");
    EXPECT_FALSE(noInputParams.prepareTemplate());
    EXPECT_FALSE(noInputParams.valid());

    AbstractVGen absentInputParam("absentiInputParam", scin::base::AbstractVGen::Rates::kPixel, false, false, { "a" },
                                  { "out" }, { { 1, 1 } }, { { 1 } }, "@out = @time * (@a + @b);");
    EXPECT_FALSE(absentInputParam.prepareTemplate());
    EXPECT_FALSE(absentInputParam.valid());

    AbstractVGen noOutputParam("absentOutputParam", scin::base::AbstractVGen::Rates::kPixel, false, false, { "a" }, {},
                               { { 1 } }, { {} }, "@out = 1.0f;");
...
TEST(AbstractVGenTest, ParameterizeInvalid) {
    AbstractVGen invalid("invalid", scin::base::AbstractVGen::Rates::kPixel, false, false, {}, {}, {}, {},
                         "@out = 1.0f;");
    EXPECT_FALSE(invalid.prepareTemplate());
    EXPECT_FALSE(invalid.valid());
    EXPECT_EQ("", invalid.parameterize({}, {}, {}, {}, {}));

    AbstractVGen mismatch("mistmatchInput", scin::base::AbstractVGen::Rates::kPixel, false, false, { "in1", "in2" },
                          { "out" }, { { 1, 1 } }, { { 1 } }, "@out = @in1 + @in2 / 2.0;");
...
TEST(AbstractVGenTest, ParameterizeValid) {
    AbstractVGen constant("constant", scin::base::AbstractVGen::Rates::kPixel, false, false, {}, { "out" }, { {} },
                          { { 1 } }, "@out = 2.0f;");
    EXPECT_TRUE(constant.prepareTemplate());
    EXPECT_TRUE(constant.valid());
    EXPECT_EQ("float subOut = 2.0f;", constant.parameterize({}, {}, { "subOut" }, { 1 }, {}));

    AbstractVGen passthrough("passthrough", scin::base::AbstractVGen::Rates::kPixel, false, false, { "in" }, { "out" },
                             { { 1 } }, { { 1 } }, "@out = @in;");
    ...
    AbstractVGen moreComplex("moreComplex", scin::base::AbstractVGen::Rates::kPixel, false, false,
