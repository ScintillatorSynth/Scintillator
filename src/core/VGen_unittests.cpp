...
TEST(VGenTest, InvalidInputCounts) {
    std::shared_ptr<const AbstractVGen> noInputs(new AbstractVGen("noInput", "@out = @time * 0.5f;", {}, {"time"}, {}));
    VGen someInputs(noInputs);
    someInputs.addConstantInput(1.0f);
    EXPECT_FALSE(someInputs.validate());

    std::shared_ptr<const AbstractVGen> twoInputs(new AbstractVGen("twoInputs", "@out = @in1 + @in2;", {"in1", "in2"},
                {}, {}));
...
TEST(VGenTest, InputValuesAndTypesRetained) {
    std::shared_ptr<const AbstractVGen> threeInputs(new AbstractVGen("threeInputs", "@out = @a + @b + @c;",
                {"a", "b", "c"}, {}, {}));
